...
```{r}
Model_df0 <- read_excel("test2.xlsx")
...
team_stats <- function(team_index){
    ...
    team_2021 <- Team_For %>% 
        ...
        select("RPG", "SPG", "TOPG", "GP", "FGA", "FG", "X3FG") %>% 
        mutate(Avg_FGA = FGA/GP)
...
predict_winner <- function(team1, team2){
    ...
    df <- c()
    df <- df %>% cbind(Diff.TRB, Diff.TOV, Diff.Avg.FGA, Diff.STL, Diff.FGp, Diff.3p) %>% as.data.frame()
